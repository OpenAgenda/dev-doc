...
| threshold      | texte\|nombre      | Seuil de pertinence minimal sur les résultats de `search`, pour écarter les correspondances faibles. Sans effet sans `search`. Valeurs: `off` (défaut, aucun filtrage), `auto` (seuil dynamique recommandé), ou un nombre (score minimum absolu).<br/>Exemple: `?search=concert&threshold=auto` |
...
| age[gte]       | entier             | Filtrer par tranche d'âge: ne remonte que les événements dont l'âge maximum est supérieur ou égal à la valeur précisée.<br/>Exemple: `?age[gte]=6`                                                                                                                                                                  |
| age[lte]       | entier             | Filtrer par tranche d'âge: ne remonte que les événements dont l'âge minimum est inférieur ou égal à la valeur précisée.<br/>Exemple: `?age[lte]=12`. Combiné avec `age[gte]`, permet de cibler les événements dont la tranche d'âge chevauche un intervalle donné (ex: `?age[gte]=6&age[lte]=12`).                  |
...
##### Comprendre l'ordre des résultats avec includeSort
