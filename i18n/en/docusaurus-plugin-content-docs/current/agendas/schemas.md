...
## The Event Schema of an Agenda

An agenda offers at creation an event form derived from a standardized event schema on the platform. The fields that make up this schema are defined under the `schema.fields` key of [an agenda's configuration](/agendas/lecture).
